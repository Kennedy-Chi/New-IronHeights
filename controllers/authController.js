...
exports.signup = catchAsync(async (req, res, next) => {
  ...
  const email = new Email(newUser, "https://ironheightsengineering.com");
  try {
    email.sendWelcome();
  } catch (err) {
    return next(
      new AppError("Sorry email can't be sent. Try again later!", 500)
    );
  }
});
...
exports.login = catchAsync(async (req, res, next) => {
  ...
  if (!username || !password) {
    console.log("username and password not exist");
